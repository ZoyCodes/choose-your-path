...
function buildEvent(records: Array<{ messageId: string; body: string }>): SQSEvent {
  return {
...
describe('webhookDelivery handler', () => {
  ...
  it('returns no failures when all deliveries succeed', async () => {
    const event = buildEvent([
      {
        messageId: 'msg-1',
        body: JSON.stringify({ eventType: 'round.closed', eventId: 'evt-1', eventVersion: 'v1' }),
      },
    ]);
  ...
  it('returns failed item identifiers for failed webhook responses', async () => {
    ...
    const event = buildEvent([
      { messageId: 'msg-1', body: JSON.stringify({ eventType: 'round.closed' }) },
      { messageId: 'msg-2', body: JSON.stringify({ eventType: 'round.opened' }) },
    ]);
  ...
  it('includes signature header when signing secret is configured', async () => {
    process.env.WEBHOOK_SIGNING_SECRET_ARN = 'arn:aws:secretsmanager:us-east-1:123456789012:secret:webhook';
  ...
  it('throws when webhook target is not configured', async () => {
    ...
    await expect(handler(buildEvent([{ messageId: 'msg-1', body: '{}' }]))).rejects.toThrow(
      'WEBHOOK_TARGET_URL is required for webhook delivery',
    );
  });
});
